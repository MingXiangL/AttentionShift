...
class TwoStageDetectorVIS(BaseDetector):
...
#             # 初始化point 位置
            ...
            mask_weight = None
            if with_mask:
                pseudo_gt_labels, pseudo_gt_bboxes, mil_losses, pseudo_gt_masks, ignore_mask = \
                    seed_gt_out[0], seed_gt_out[1], seed_gt_out[2], seed_gt_out[3], seed_gt_out[4]
                gt_masks = pseudo_gt_masks
                mask_weight = ignore_mask
                if self.visualize:
                    points_attn_maps_per_img = seed_gt_out[-1]
                    org_image = 
            else:
