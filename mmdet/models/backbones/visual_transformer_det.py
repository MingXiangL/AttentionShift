...
class VisionTransformerDet(VisionTransformer):
    def __init__(self,
        ...
        self.with_point_head = with_point_head
        if self.with_point_head:
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_checkpoint:
                if self.return_attention:
                    ...
                    # attns.append(std_reweighting(attn))
        ...
        org_features = torch.stack(features, dim=1)
        if self.with_fpn:
        ...
        ret_dict = dict(org_feats=org_features, 
        ...
        if self.with_point_head:
            ...
            ret_dict.update(dict(outputs_class=outputs_class, outputs_coord=outputs_coord))
        
        
        if self.return_attention and self.last_feat:
            ret_dict.update(dict(attns=attns))
        if self.last_feat:
            ret_dict.update(dict(last_feat=last_feat))
