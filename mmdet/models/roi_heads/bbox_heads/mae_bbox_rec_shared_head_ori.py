...
#         self.rec_norm = norm_layer(embed_dim)
#         self.rec_pred = nn.Linear(embed_dim, embed_dim, bias=True) # decoder to feature
        self.rec_token_norm = norm_layer(embed_dim)
...
# ## rec_loss
#         loss_rec = (x_rec - x_bbox.detach()) ** 2
#         loss_rec = loss_rec.mean()  # [N, L], mean loss per patch
#         loss_rec = self.loss_weight * loss_rec 
#         losses['loss_rec'] = loss_rec
        
#         dis_score = self.dis_layer(x_det_token)
#         gen_score = self.dis_layer(x_rec_token)
        
        
#         loss_gan_dis = - torch.mean(
#             torch.log(dis_score) + torch.log(1. - gen_score)
#         )
#         loss_gan_gen = -torch.mean(torch.log(1. - gen_score)
#         )
        
#         losses['loss_gan_gen'] = loss_gan_gen
#         losses['loss_gan_dis'] = loss_gan_dis
    ...
    def forward(self, x):
        ...
        x = torch.cat([self.det_token.expand(2*B, -1, -1), x], dim=1)
...
#         x_rec=self.rec_norm(x[B:,1:,:])
#         x_rec=self.rec_pred(x_rec)[mask]
        
        x_rec_token = self.rec_token_norm(x[B:, 0, :])
#         x_rec_token = self.decoder_box_norm(x[B:, 0, :])
