...
#             print(self.bbox_head.seed_score_thr, self.bbox_head.cam_layer)
            ...
            for c, p in zip(cam_, pseudo_point_locations):
                ...
                pseudo_gt_bbox = torch.as_tensor(pseudo_gt_bbox, device=point_cls.device, dtype=torch.float16)
...
#                     scale_cam = cv2.resize(scale_cam, (patch_w * 16, patch_h * 16))
                    ...
                    scale_bboxes.append(torch.as_tensor(pseudo_gt_bbox, dtype=gt_points[0].dtype, device=device))
        ...
        if self.with_mil:
            gt_bboxes, mil_losses = self._mil_forward_train(roi_feature_map, None,
                                                    gt_bboxes, gt_labels,
                                                    img_metas)
            
        return gt_labels, gt_bboxes, mil_losses
...
#                     else:
            ...
            cls_score = torch.zeros_like(cls_scores, dtype=torch.float16, deviec=bbox_feats.device)
