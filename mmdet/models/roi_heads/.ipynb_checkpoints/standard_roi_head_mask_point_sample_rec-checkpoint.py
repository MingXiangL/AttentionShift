...
class StandardRoIHeadMaskPointSampleRec(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
...
#         return coords_chosen, labels_chosen
    ...
    #     return coords_chosen, labels_chosen
    def get_mask_sample_points_roi(self, attn, rois, pos_thr=0.2, neg_thr=0.05, num_gt=20):
        # Parameters:
        #     attn: num_layers, num_points, H, W
        #     roi: num_points, 4
        # Return:
        ...
        for i_p, attn_p in enumerate(attn_map):
            ...
            coor, label = self.get_mask_points_single_instance(attn_crop, pos_thr=pos_thr, neg_thr=neg_thr, num_gt=num_gt,i=i_p)
            coor[:, 0] += ymin
            coor[:, 1] += xmin
            coor = coor.flip(1)
            coord_chosen.append(coor)
    ...
    def get_mask_sample_points_roi_best_attn(self, attn, rois, attn_idx, pos_thr=0.2, neg_thr=0.05, num_gt=20):
        # Parameters:
        #     attn: num_layers, num_points, H, W
        #     roi: num_points, 4
        # Return:
    ...
    def get_mask_points_single_instance(self, attn_map, pos_thr=0.1, neg_thr=0.01, num_gt=10,i=0):
        # Parameters:
        #     coords: num_pixels, 2
        #     attn:H, W
        #     cls: scalar,
...
#                                  cam_layers=-torch.arange(1, 13), patch_size=(patch_h, patch_w))
        coords_map = self.get_map_coords(patch_h*16, patch_w*16, device=x[2].device, dtype=x[2].dtype)
...
#                     scale_cam = cv2.resize(scale_cam, (patch_w * 16, patch_h * 16))
                    ...
                    scale_bboxes.append(torch.as_tensor(pseudo_gt_bbox, dtype=gt_points[0].dtype, device=gt_points[0].device))
        ...
        if return_mask:
            ...
            for i_img in range(num_imgs):
                # coord_point, labels_point = self.get_mask_sample_points_roi(attn_maps_dealed[i_img], mil_out[0][i_img], pos_thr=pos_mask_thr, neg_thr=neg_mask_thr, num_gt=num_mask_point_gt)
                coord_point, labels_point = self.get_mask_sample_points_roi_best_attn(attn_maps_dealed[i_img], mil_out[0][i_img], gt_box_index[i_img], pos_thr=pos_mask_thr, neg_thr=neg_mask_thr, num_gt=num_mask_point_gt)
            ...
            return gt_labels, mil_out[0], mil_out[1], gt_mask_points_coords, gt_mask_points_labels, points_attn_maps_per_img
        else:
...
#         return mask_results
    ...
    def _mask_forward_train(
        self, x, sampling_results, bbox_feats, points_coords, points_labels, img_metas=None, **kwargs
    ):
