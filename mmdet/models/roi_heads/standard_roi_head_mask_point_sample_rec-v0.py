...
#                     scale_cam = cv2.resize(scale_cam, (patch_w * 16, patch_h * 16))
        ...
        for i_img in range(num_imgs):
            # coord_point, labels_point, map_cos_fg, map_cos_bg, sim_fg, sim_bg, uncertain_map, coord_semantic_center = self.get_mask_sample_points_roi_best_attn_local_global(attn_maps_dealed[i_img], mil_out[0][i_img], gt_box_index[i_img], 
            #                                             vit_feat=vit_feat[i_img], pos_thr=pos_mask_thr, neg_thr=neg_mask_thr, num_gt=num_mask_point_gt, gt_points=center_points[i_img],
            #                                             mean_shift_refine=False, gt_labels=gt_labels[i_img])
            # coord_point, labels_point, map_cos_fg, map_cos_bg = self.get_mask_sample_points_roi_prots_best_attn_feat_refine(attn_maps_dealed[i_img], mil_out[0][i_img], gt_box_index[i_img], 
            #                                             vit_feat=vit_feat[i_img], pos_thr=pos_mask_thr, neg_thr=neg_mask_thr, num_gt=num_mask_point_gt, obj_tau=obj_tau)
            ...
            semantic_centers, semantic_centers_split, sim_fg = self.get_semantic_centers(map_cos_fg[-1].clone(), 
                                                        map_cos_bg[-1].clone(), 
                                                        mil_out[0][i_img], 
            ...
            map_cos_fg_ret.append(map_cos_fg)
            map_cos_bg_ret.append(map_cos_bg)
        ...
        if self.visualize:
            return dict(pseudo_gt_labels=gt_labels,
                        ...
                        semantic_centers_ret=semantic_centers_ret,
                        points_bg=points_bg, 
                        points_fg=points_fg,
                        sim_fg=sim_fg_ret,
