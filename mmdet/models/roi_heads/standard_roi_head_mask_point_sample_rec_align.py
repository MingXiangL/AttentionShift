...
class StandardRoIHeadMaskPointSampleRecAlign(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        if self.with_align:
            self.object_queues = ObjectQueues(num_class=self.bbox_head.num_classes,
                                              len_queue=len_queque, ratio_range=ratio_range, appear_thresh=appear_thresh, max_retrieval_objs=max_retrieval_objs)
        ...
        if deform_attn_head is not None:
            self.deform_attn = build_head(deform_attn_head)
            self.with_deform_attn = True
        else:
            self.with_deform_attn = False
...
#             losses.update(bbox_results['loss_bbox_rec'])
        ...
        if self.with_align:
            ...
            for i_img, res in enumerate(point_assign_results):
                token_img = feats_point_tokens[i_img, res.pos_inds]
                tokens_pos.append(token_img)
            ...
            loss_align = self.align_forward_train(semantic_centers_feat_split, tokens_pos, gt_bboxes_arrange, gt_labels_arrange)
            losses.update(loss_align)

        if self.with_keypoint_align:
            tokens_pos = []
            feats_parts = []
            part_obj_labels = []
            has_part = False
            obj_pre = 0
            for i_img, res in enumerate(point_assign_results):
                for i_obj in range(res.pos_inds.shape[0]):
                    tokens_pos.append(feats_point_tokens[i_img, res.pos_inds[i_obj]].detach())
                    if len(semantic_centers_feat_split[i_img]) > 0:
                        if torch.numel(semantic_centers_feat_split[i_img][i_obj]) > 0:
                            feats_parts.append(semantic_centers_feat_split[i_img][i_obj])
                            part_obj_labels += [obj_pre] * semantic_centers_feat_split[i_img][i_obj].shape[0]
                            has_part = True
                    obj_pre += 1
            if has_part:
                feats_parts = torch.cat(feats_parts, dim=0)
                part_obj_labels = torch.tensor(part_obj_labels, dtype=torch.long, device=feats_parts.device)
            loss_keypoint_align = self.keypoint_align_head(torch.stack(tokens_pos, dim=0), feats_parts, part_obj_labels)
            losses.update(loss_keypoint_align)

        # if self.with_deform_attn:
        #     # TODO:支持batch操作
        #     vit_feat_rs = vit_feat[:, 1:].permute(0,2,1).unflatten(-1, x[-1].shape[-2:])

        #     point_idx = (semantic_centers_org[0][0] / 16 - 0.5).floor().long().flip(1)
        ...
        #     else:
        #         if self.visualize:
        #             loss_deform, keypoint_offset, reference, assets = self.deform_attn(vit_feat_rs, point_idx, semantic_centers_org[1][0], return_offset=True)
        #             self.keypoint_offset = keypoint_offset
        #             self.reference = reference
        #             self.assets = assets
        #         else:
        #             loss_deform = self.deform_attn(vit_feat_rs, point_idx, semantic_centers_org[1][0])
        ...
        if self.with_deform_attn:
            ...
            for i_img in range(num_imgs):
                ...
                if torch.numel(point_idx) == 0:
                    loss_deform_tmp = self.deform_attn(
                        torch.zeros_like(vit_feat_rs), 
                        torch.zeros(1, 2, dtype=torch.long, device=vit_feat_rs.device), 
                        torch.zeros(1, dtype=torch.long, device=vit_feat_rs.device),
                        torch.zeros(1, 2, dtype=torch.float, device=vit_feat_rs.device),
                        all_zero=True)
                else:
                    if self.visualize:
                        loss_deform_tmp, keypoint_offset, reference, assets = self.deform_attn(vit_feat_rs, point_idx, semantic_centers_org[1][0], point_coords, return_offset=True)
                        self.keypoint_offset = keypoint_offset
                        self.reference = reference
                        self.assets = assets
                    else:
                        loss_deform_tmp = self.deform_attn(vit_feat_rs, point_idx, semantic_centers_org[1][0], point_coords)
