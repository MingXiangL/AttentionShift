...
class StandardRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def init_weights(self, pretrained):
        ...
        if self.with_mask:
            if hasattr(self.mask_head, 'pretrained'):
                self.mask_head.init_weights(pretrained=pretrained)
            else:
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            #     losses.update(bbox_results['loss_bbox']['loss_rec'])
    ...
    def _bbox_forward(self, x, rois, loss_guide=False, 
                      pos_inds=None, loss_bbox_items=None, loss_cls_items=None):
...
#             bbox_results = dict(
#             cls_score=cls_score, bbox_pred=bbox_pred, bbox_feats=bbox_feats, rec_cls_score=rec_cls_score, rec_bbox_pred=rec_bbox_pred)
            if loss_guide:
                cls_score, bbox_pred = self.bbox_head(bbox_feats, mask=loss_guide, 
                                                     pos_inds=pos_inds,
                                                     loss_bbox_items=loss_bbox_items,
                                                     loss_cls_items=loss_cls_items,
                                                     )
                bbox_results = dict(
                    rec_cls_score=cls_score, rec_bbox_pred=bbox_pred)
            else
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        if self.with_bbox_rec:
            loss_bbox, pos_inds, loss_bbox_items, loss_cls_items = self.bbox_head.loss(bbox_results['cls_score'],
                                            bbox_results['bbox_pred'],
                                            rois,
                                            *bbox_targets)
            # 根据loss指导 mask的比例
            ...
            loss_bbox_rec = self.bbox_head.loss_(bbox_results['rec_cls_score'],
