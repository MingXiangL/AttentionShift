...
#         assert with_cls or with_reg
        ...
        if loss_reconstruction is not None:
            self.loss_reconstruction = build_loss(loss_reconstruction)
        ...
        self.loss_point_cls = build_loss(loss_point_cls)
    ...
    def crop_feature(self, vit_feat, rois):
        batch_inds = rois[:, 0]
        rois_ = rois[:, 1:]
        roi_feats = []
        for ind, roi in zip(batch_inds, rois_):
            feat = vit_feat[ind.long()]
            grid_height, grid_width = feat.size()[-2:]
            feat_grid = roi.round().int()   # xmin, ymin, xmax, ymax
            # 将边界拉到框内
#             print(feat_grid)
            feat_grid[0::2].clamp_(min=0, max=grid_width-1)  # x 拉到 width内
            feat_grid[1::2].clamp_(min=0, max=grid_height-1)  # y 拉到 height内
            # 将数据直接带入arg进行采样
            xmin, ymin, xmax, ymax = feat_grid
            roi_feat = feat[:, ymin:ymax+1, xmin:xmax+1] #.unsqueeze(0)
