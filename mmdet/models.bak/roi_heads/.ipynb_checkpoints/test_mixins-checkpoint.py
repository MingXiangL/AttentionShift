    ...
    def simple_test_bboxes(self,
        ...
        if bbox_pred is not None:
            ...
            if isinstance(bbox_pred, torch.Tensor):
                ...
                # For SABL
                bbox_preds = self.bbox_head[0].bbox_pred_split(
                    bbox_pred, num_proposals_per_img) if isinstance(self.bbox_head, nn.ModuleList) else self.bbox_head.bbox_pred_split(
                ...
                for i in range(len(proposals)):
                    ...
                    det_bbox, det_label = self.bbox_head[0].get_bboxes(
                        ...
                        scale_factors[i],
                        rescale=rescale,
                        cfg=rcnn_test_cfg) if isinstance(self.bbox_head, nn.ModuleList) else self.bbox_head.get_bboxes(
                                                        rois[i],
                                                        cls_score[i],
                                                        bbox_preds[i],
                                                        img_shapes[i],
        ...
        return self.bbox_head[0].get_bboxes(
            rois,
            cls_score,
            bbox_pred,
            img_shapes,
